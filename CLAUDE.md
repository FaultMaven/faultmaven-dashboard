...
- **AdminCaseListPage**: Cross-tenant "All Cases" list (ADR-012 D9) — every user's cases on the server (Copilot- and Slack-agent-originated). Backed by `GET /api/v1/admin/cases`; state/source filters only. **The route and the nav item are gated separately, on purpose.** The ROUTE keeps `canViewAllCases(isAdmin)` → `platform_admin` in both deployments, matching the backend. The NAV ITEM asks `offersAllCasesNav(deployment, isAdmin)`, which withholds it in standalone because there it is a **copy of `Cases`**: the bootstrap creates ONE account and re-grants IT the operator roles every startup (`ensure_default_admin_exists` → `assign_operator_roles`), and the `full` arm then serves that account the list it already has — measured on a live stack, `GET /cases` and `GET /admin/cases` returned the same 21 ids. ‼ **Standalone is single-user by design** — one person running it locally, which is how it is positioned and expected to be used — so that is the whole population, not one install's luck. It can technically hold more accounts (`faultmaven.sh create-user` makes ordinary `["user"]` ones) and only the bootstrap account is the operator, which is the single shape where this view would show them something `Cases` cannot; **multi-user standalone is not a supported configuration** and must not earn a nav slot, a capability flag or a branch — carrying a non-use-case as "a limitation to fix later" is how UI and configuration accrete around it. Only the OFFER moves because the ROUTE mirrors the BACKEND, which still serves `/admin/cases` in standalone under D9's standing (recorded, not gated) access — narrowing what the nav advertises is not a decision about what the deployment serves. ‼ The spelling is `!== 'standalone'`, never `=== 'cloud'`: `isAdmin` is available synchronously from stored auth state but `deployment` needs a `/auth/config` round trip that `loading` does not cover, so the nav renders with `deployment === null` on every hard refresh and for as long as that endpoint is unreachable — and requiring a confirmed `'cloud'` would take the item from the CLOUD operator in both windows, which this standalone-scoped fix must not do. Cloud is byte-identical to the old gate; the cost is that a standalone nav shows the item briefly before detection resolves. The route half is bound on App's real route table by `src/test/pages/adminCasesRoute.test.tsx` — a hand-built router tests the guard component and not the wiring, which is how `ChatSurfaceRoute`'s deletion once left 1200 tests green. The response is a union **discriminated on `view`**, and the page narrows on it rather than on the deployment mode, so rendered columns cannot drift from served policy: `view: "full"` (standalone) renders `CaseTable` with titles; `view: "metadata"` (cloud) renders `AdminCaseMetadataTable` — ids/state/timestamps/counts, **no title or description** (user free text is content and needs the audited break-glass path, faultmaven#815). The endpoint still 403s under `TENANT_PROVIDER=multi` (RLS would make the list silently partial); the page shows that refusal *instead of* a table. Rows on **both** arms open through `/admin/cases/{id}` (the audited operator read), never `/cases/{id}` — the latter has no operator bypass and 404s on cases the operator does not own (faultmaven#846). The ENTERPRISE travels on the link (`?enterprise=`) because requesting a grant needs it — `BreakGlassGrantRequest.enterprise_id`, the isolation tenant (ADR-017 D1), never the billing organization.
